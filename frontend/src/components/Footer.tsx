...
// Formatea un número estilo "+54 9 11 7634-8089" a partir de solo dígitos
const formatPhone = (raw: string): string => {
  const d = (raw || '').replace(/\D/g, '');
  const m = d.match(/^549?(\d{2})(\d{3,4})(\d{3,4})$/);
  if (!m) return raw ? `+${d}` : '';
...
export default function Footer() {
  ...
  // Categorías dinámicas desde la API + secciones fijas
  const linksGaleria = [
